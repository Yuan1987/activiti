...
<div class="modal-dialog">
	<div class="modal-content">
		...
		<input id="id" name="id" value="${as.id }" type="hidden">
			<div class="form-body">
					...
					<label class="col-sm-2 control-label">*能力类别</label>
                        <div class="col-sm-8">
                                ...
                                <option></option>
                                	...
                                	<option value="${dic.code}" <c:if test="${dic.code ==as.typeid}">selected</c:if>>${dic.detail}</option>
                                	</c:forEach>
                                </select>
                        </div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-8">
						<input type="text" id="name" name="name" class="form-control" required="required" value="${as.name}">
					</div>
				</div>
				<div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <select class="form-control sp"   id="serial" name="serial" required="required">
                            ...
                            <c:forEach items="${xlList}" var="dic">
                                <option value="${dic.code}" <c:if test="${dic.code ==as.serial}">selected</c:if>>${dic.code}|${dic.detail}</option>
                            </c:forEach>
                        </select>
                    </div>
				</div>
				...
				<label class="col-sm-2 control-label">*所属级别</label>
                    <div class="col-sm-8">
                        <select class="form-control sp"  name="thelevel"   id="thelevel" required="required">
                            <option></option>
                            <c:forEach items="${jbList}" var="dic">
                                <option value="${dic.code}" <c:if test="${dic.code ==as.thelevel}">selected</c:if>>${dic.code}|${dic.detail}</option>
                            </c:forEach>
                        </select>
                    </div>
				</div>
				<div class="form-group">
					<label class="col-sm-2 control-label">*能力分值</label>
                        <div class="col-sm-8" >
                           <input type="text" class="form-control" name="score" value="${as.score }" required="required">
                        </div>
				</div>
				<div class="form-group">
                           ...
                           <select class="form-control"  name="formula"   id="formula" required="required">
                              <option></option>
                                  <c:forEach items="${dics_formula}" var="dic">
                                     <option value="${dic.code}" <c:if test="${dic.code ==as.formula}">selected</c:if>>${dic.detail}</option>
                                  </c:forEach>
                          </select>
                  	</div>
				</div>
				...
				<label class="col-sm-2 control-label">*权重</label>
                     <div class="col-sm-8">
                          <input type="text" class="form-control" name="passmark" value="${as.passmark}" required="required">
                     </div>
				</div>
				<div class="form-group">
				<label class="col-sm-2 control-label">*证据的输出件</label>
                     <div class="col-sm-8">
                          <textarea  class="form-control" name="evidence"  rows="3"  maxlength="500"  required="required">${as.evidence}</textarea>
                     </div>
				</div>
			</div>
...
<script type="text/javascript" src="${ctx}/static/model/sys_ability_edit.js"></script>
